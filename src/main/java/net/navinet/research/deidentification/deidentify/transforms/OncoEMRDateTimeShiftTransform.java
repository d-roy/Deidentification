...
final class OncoEMRDateTimeShiftTransform extends AbstractDataFrameTransform {
  static final Logger logger = LogManager.getLogger(OncoEMRDateTimeShiftTransform.class.getName());
  ...
  OncoEMRDateTimeShiftTransform(String columnName) {
    super(columnName);
  }

  static void register(SQLContext sqlContext) {
    sqlContext.udf().register("DateTimeShiftTransform", new UDF2<Object, Object, String>() {
      @Override
  ...
  public DataFrame transform(DataFrame input) {
    return input.withColumn(columnName, callUDF("DateTimeShiftTransform", input.col(columnName),
        input.col("patientid")).cast("timestamp"));
  }
}
